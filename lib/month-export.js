...
const { resolveCisloItt, buildRecipeMap } = require('./recipe-normalize');
...
// recipeMap: mapa směs→{cislo,zt} TÉ obalovny (B2). null → B2 vypnuto (zpětná kompat).
function buildMonthSheet(wb, y, m0, entries, companyArgb, recipeMap) {
  ...
  // ── Datové řádky ──
  rows.forEach(r => {
    // B2 (svaté pravidlo): cislo/itt ŽIVĚ z receptury dle smes. Osiřelá smes (mimo receptury)
    // → uložená hodnota zůstane + buňky č./směs/ITT se obarví červeným písmem.
    const res = recipeMap ? resolveCisloItt(r.smes, r.cislo, r.itt, recipeMap) : { cislo: r.cislo, itt: r.itt, osirela: false };
    const vals = [res.cislo || '', r.lokalita || '', r.objednavka || '', r.smes || '', res.itt || '', r.ceta || ''];
    let rowTotal = 0;
    ...
    row.eachCell({ includeEmpty: true }, (c, col) => {
      ...
      if (col <= nInfo) {
        ...
        // B2: osiřelá směs → červené písmo na č.(1) / směs(4) / ITT(5)
        if (res.osirela && (col === 1 || col === 4 || col === 5)) c.font = { color: { argb: 'FFDC2626' }, bold: true };
      } else {
        c.alignment = { horizontal: 'center', vertical: 'middle' };
...
// Hlavní vstup: { weeks, companies, year, inputs } → ExcelJS.Workbook s 12 listy.
// `inputs` = receptury TÉ obalovny (B2). Multi-tenant: VŽDY předat receptury téže obalovny jako
// `weeks`. Když `inputs` chybí (undefined), B2 se vypne (recipeMap=null) → identické chování.
function buildMonthWorkbook({ weeks, companies, year, inputs }) {
  const entries = buildEntries(weeks);
  const recipeMap = Array.isArray(inputs) ? buildRecipeMap(inputs).map : null;
  ...
  wb.created = new Date();
  for (let m0 = 0; m0 < 12; m0++) buildMonthSheet(wb, year, m0, entries, companyArgb, recipeMap);
  return wb;
}
